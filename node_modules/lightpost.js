        ...
        this.handle_char = function(c)
        {
            ...
            else
            {
                if (c === ' ' || c === '\n' || c === '\t' || c === '\r')
                {
                    if (layer.word !== '')
                    {
                        if (layer.word.length > 2 && layer.word.substr(layer.word.length - 2) === ')#')
                        {
                            layer.next_layer.handle_word(layer.word.substr(0, layer.word.length - 2));
                            layer.next_layer.handle_word(')#');
                        }
                        else if (layer.word.length > 1 && layer.word[layer.word.length - 1] === ')')
                        {
                            layer.next_layer.handle_word(layer.word.substr(0, layer.word.length - 1));
                            layer.next_layer.handle_word(')');
                        }
                        else
                        {
