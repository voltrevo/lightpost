    ...
    this.builtin_functions =
    {
        ...
        '!':
        {
            ...
            exec: function(x)
            {
                if (typeof x !== 'boolean')
                {
                    self.stderr('Error: !: argument must be a boolean');
                }
        ...
        'inspect':
        {
            ...
            exec: function()
            {
                self.stdout('Stack: ' + JSON.stringify(self.innermost_exec_layer.stack));
                self.stdout('Variables: ' + JSON.stringify(self.innermost_exec_layer.variables));
                self.stdout('Module Variables: ' + JSON.stringify(self.module_variables));
            }
        },
        'exec':
        {
        ...
        '=':
        {
            ...
            exec: function()
            {
                ...
                {
                    self.stderr('Error: Not enough arguments for assignment');
        ...
        */
        'floor':
        {
            ...
            exec: function(x)
            {
                if (typeof x !== 'number')
                {
                    self.stderr('Error: floor: argument is not a number');
                    return;
                }
        ...
        'str_to_n':
        {
            ...
            exec: function(x)
            {
                if (typeof x !== 'string')
                {
                    self.stderr('Error: str_to_f: argument is not a string');
                    return;
                }
    ...
    this.comment_layer = new (function()
    {
        ...
        this.block_comment_mode = function(c)
        {
            if (layer.had_slash)
            {
                ...
                {
                    layer.block_depth++;
                    //self.stdout('incremented block depth to ' + layer.block_depth);
                }
                else if (c === '\n')
                {
            ...
            else if (layer.had_star)
            {
                ...
                {
                    layer.block_depth--;
                    //self.stdout('decremented block depth to ' + layer.block_depth);
                ...
                if (c === ' ' || c === '\n' || c === '\t' || c === '\r')
                {
                    if (layer.word !== '')
                    {
                        ...
                        for (var i in words)
                        {
                            ...
                            {
                                self.stderr('Error: Cannot parse word "' + words[i] + '"');
                            }
                            else
                            {
    ...
    this.create_exec_layer = function() { return new (function()
    {
        ...
        this.handle_word = function(word) {layer.add_task(function()
        {
            ...
            else if (word[word.length - 1] === '#')
            {
                ...
                {
                    self.stderr('Error: No matching symbol for "' + sub_word + '"');
                }
            }
        ...
        this.exec = function(fn)
        {
            ...
            {
                self.stderr('Error: Not enough arguments for function (' + layer.stack.length + ' < ' + fn.argc + ')');
